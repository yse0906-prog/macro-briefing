...
- **본문 강조 표시**(섹터·브리핑 공통): 비교 문장과 핵심 결론이 눈에 띄게 표시한다.
  - `**조건·핵심 결론**` → 굵게. 예) `**갭이 음(−)이면**`, `**금리보다 주가**`
  - `[[+유리한 결과]]` → 초록, `[[-불리한 결과]]` → 빨강. 해당 업권·섹터 입장에서 좋고 나쁨이 분명할 때만 쓴다. 예) `[[+순자산이 늘어난다]]`, `[[-헤지 비용이 늘었다]]`
  - 원화 강세·약세처럼 좋고 나쁨이 정해지지 않은 비교는 색 없이 굵게만 쓴다.
  - 문단마다 1~3곳만 표시한다. `headline`, `numbers[].text`의 수치, `factor`·`name` 같은 이름 칸에는 쓰지 않는다.
  - 짝이 맞지 않으면 검사에서 실패한다.
...
- **하위 에이전트(Agent 도구)를 띄우지 않는다.** 한 섹터씩 순서대로 직접 조사한다. 검색을 병렬로 몰아서 하면 사용량 한도에 걸려 루틴이 중간에 멈춘다(2026-09-21 실제 발생).
- 섹터당 검색은 3~4회 안팎으로 한다. 필요한 숫자를 찾으면 더 검색하지 않는다.
