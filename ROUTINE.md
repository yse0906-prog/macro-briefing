...
- `institution_impact`에는 **증권사(`securities`)·LP(`lp`)·GP(`gp`) 세 항목을 모두** 넣는다. 은행권 하나로 뭉뚱그리지 않는다. weekly·event 모두 해당한다.
  - 각 업권은 `text`(한두 문장 요약)와 `points`를 갖는다. `points`는 아래 고정 항목을 **모두** 채운다. 운용자산 관점에서, 이번 주 금리·환율·주가 변화가 그 업권의 손익과 건전성에 어떻게 들어오는지 쓴다.
    ...
    - LP: `duration_gap`(보험사 자산·부채 듀레이션 갭, 금리 변화가 순자산에 주는 방향, LTFR 외삽 구간 주의) · `kics`(K-ICS 비율: 가용자본과 요구자본 중 금리·주식·외환위험액 변화, 최신 금감원 발표치) · `allocation`(환헤지 후 수익률로 본 국내 장기채 vs 해외채권 배분) · `fx_hedge`(환헤지 비용, 스와프 만기 구조, 국민연금 외환스와프·전략적 환헤지)
